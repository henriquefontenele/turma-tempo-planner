...
const PENDING_QR_KEY = 'fidelidade_pending_checkin_qr';
...
export default function CheckinEventoPage() {
  ...
  const processarEventoId = useCallback(async (eventoId: string) => {
    if (!user || processandoRef.current) return;
    ...
    try {
      ...
      if (checkinExistente.exists()) {
        ...
        setMensagem('Você já fez check-in neste evento. Os pontos só são creditados uma vez.');
        return;
      }
      ...
      try {
        ...
        if (fidUsuarioSnap.exists()) {
          ...
          try {
            ...
            console.warn('Transação registrada; cache de saldo fica para reconciliação:', e);
          }
        }
      } catch (e) {
      ...
      setMensagem(
        transacaoRegistrada
          ? `Check-in realizado! +${evento.pontosCreditar} pontos (crédito único neste evento).${saldoAtualizado ? '' : ' Veja em Meus Pontos.'}`
      ...
      setResultado('erro');
      setMensagem('Erro ao processar o check-in.');
    } finally {
      processandoRef.current = false;
    }
  }, [user, userProfile]);

  const processarQRCode = useCallback(async (data: string) => {
    try {
      ...
      await processarEventoId(parsed.eventoId);
    } catch (err) {
  ...
  // Após login: retoma QR pendente ou eventoId da URL
  useEffect(() => {
    if (loading || !user || resultado) return;

    const pending = sessionStorage.getItem(PENDING_QR_KEY);
    if (pending) {
      sessionStorage.removeItem(PENDING_QR_KEY);
      void processarQRCode(pending);
      return;
    }

    const eventoId = searchParams.get('eventoId');
    if (eventoId) {
      void processarEventoId(eventoId).then(() => {
        setSearchParams({}, { replace: true });
      });
    }
  }, [user, loading, resultado, searchParams, setSearchParams, processarEventoId, processarQRCode]);
    ...
    try {
      ...
      await scanner.start(
        ...
        async (decodedText) => {
          ...
          setScanning(false);
          if (!user) {
            sessionStorage.setItem(PENDING_QR_KEY, decodedText);
            toast({ title: 'Faça login', description: 'Após o login o check-in será concluído.' });
            return;
          }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 gap-4">
        <div className="text-center max-w-md space-y-2">
          <h1 className="text-xl font-bold">Check-in de Evento</h1>
          <p className="text-sm text-muted-foreground">
            Faça login para computar os pontos na sua conta. Os pontos de cada evento só são creditados uma vez.
          </p>
        </div>
      ...
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Escaneie o QR Code do evento. Os pontos só são creditados uma única vez por evento.
          </p>
        </div>
        ...
        {resultado === 'sucesso' && (
          <Card className="border-green-500">
            <CardContent className="flex flex-col items-center gap-4 py-8">
              ...
              <p className="text-lg font-semibold text-green-600 text-center">{mensagem}</p>
        ...
        {resultado === 'duplicado' && (
          <Card className="border-yellow-500">
            <CardContent className="flex flex-col items-center gap-4 py-8">
              ...
              <h2 className="text-xl font-bold text-yellow-700">Já creditado</h2>
